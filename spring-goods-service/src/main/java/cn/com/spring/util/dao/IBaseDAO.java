package cn.com.spring.util.dao;
...
public interface IBaseDAO<K,V> {
	 ...
	 * @return 如果有数据则数据以List集合返回，如果没有数据则集合长度为0（size()==0，不是null）
	 */
	public List<V> findAll(Long currentPage,Integer lineSize) ;
	/**
	 ...
	 * @return 如果有数据则数据以List集合返回，如果没有数据则集合长度为0（size()==0，不是null）
	 */
	public List<V> findSplit(String column,String keyWord,Long currentPage,Integer lineSize) ;
	/**
